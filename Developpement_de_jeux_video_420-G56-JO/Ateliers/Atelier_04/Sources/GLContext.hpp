...
public:
    ...
    GLContext(const char *title = "", int x = SDL_WINDOWPOS_CENTERED, int y = SDL_WINDOWPOS_CENTERED, int width = DEFAULT_WINDOW_WIDTH, int height = DEFAULT_WINDOW_HEIGTH, unsigned int windowFlags = 0) {
        sdlWindow = SDL_CreateWindow(title, x, y, width, height, windowFlags | SDL_WINDOW_OPENGL | SDL_WINDOW_RESIZABLE);
    ...
    /// @brief changement pour mettre la denêtre en plein écran
    void setFullScreen(bool fullscreen) {
        SDL_SetWindowFullscreen(sdlWindow, fullscreen ? SDL_WINDOW_FULLSCREEN : 0);
    }

    /// @brief Changement de la taille de la fenêtre.
