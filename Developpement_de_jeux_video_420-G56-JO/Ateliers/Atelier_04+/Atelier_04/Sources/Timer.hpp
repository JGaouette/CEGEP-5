...
/// @class Timer
/// @brief minuteur.
class Timer : public IEventElement {
private:
    steady_clock::time_point pauseTime;   ///< Temps en pause du minuteur.
    steady_clock::time_point endTime;     ///< Fin du minuteur.
    double time;                          ///< Temps du minuteur en secondes.
    bool isPaused;                        ///< Si le minuteur est en pause.
...
public:
    ...
    void stop() {
        if (!isPaused) {
            ...
            // Si le minuteur est terminé, on déclenche l'action.
    ...
    /// @brief Change le temps du minuteur.
    /// @param time Nouveau temps du minuteur.
    ...
    double getRemainingTime() {
        if (duration_cast<nanoseconds>(endTime - steady_clock::now()).count() <= 0 && !isPaused) {
            stop();
            return 0;
        }
